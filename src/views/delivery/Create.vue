<template>
	<div class="main">
		...
		<div class="steps-content">
				...
				<a-form-model :model="form" :label-col="labelCol" :wrapper-col="wrapperCol">
					<a-form-model-item label="选择司机" required>
						<a-select v-model="selectDriverIndex" placeholder="请选择配送司机">
							<a-select-option :value="index" v-for="(item, index) in drivers" :key="index"
								:disabled="item.driving">
								{{ item.name }}
					...
					<a-form-model-item label="预计交货时间" required>
						<a-date-picker v-model="form.time" show-time type="date" placeholder="选择日期"
							style="width: 100%;" />
					</a-form-model-item>
			...
			<div v-if="current === 2">
				<a-result status="success" title="提交成功" sub-title="请等待管理员审核配送请求。">
					<template #extra>
